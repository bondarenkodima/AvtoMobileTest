...
public class MainPageObject {
    ...
    public void swipeUp(int timeOfSwipe)   // скрол страницы по y
    {
        if (driver instanceof AppiumDriver){
            ...
            System.out.println("Method swipeApp() does nothing for platform" + Platform.getInstance().getPlatformVar());
        }
    ...
    public void scrollWebPageUp()  // скрол для web
    {
        if (Platform.getInstance().isMW()){
            JavascriptExecutor JSExecutor = (JavascriptExecutor) driver;  // исполнение джава скрипт натекущей странице
            JSExecutor.executeScript("window.scrollBy(0, 250)");  //  скрол с 0 позиции по 250 пиксел
        } else {
            System.out.println("Method scrollWebPageUp() does nothing for platform" + Platform.getInstance().getPlatformVar());
        }
    }

    public void scrollWebPageTitleElementNotVisible(String locator, String error_message, int max_swipes) // делает скрол пока нужные элемент не будет в поле зрения
    {
        int already_swiped = 0;

        WebElement element = this.waitForElementPresent(locator, error_message);

        while (!this.isElementLocatedOnTheScreen(locator)) // ! означает отрицание,тоесть пока элемент не на экране
            {
                scrollWebPageUp();
                ++already_swiped;
